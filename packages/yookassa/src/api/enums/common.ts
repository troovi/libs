...
export enum PaymentStatusEnum {
  /**
   * pending — платеж создан и ожидает действий от пользователя. В некоторых случаях платеж может оставаться в этом статусе даже после подтверждения платежа пользователем:
   * Если вы используете стороннюю онлайн-кассу для работы по 54-ФЗ и сценарий Сначала чек, потом платеж, то платеж может находиться в статусе pending до тех пор, пока онлайн-касса не сообщит об успешной или неуспешной регистрации чека.
   * Если вы проводите платеж способом «Покупки в кредит» от СберБанка и применяется охлаждение кредита или рассрочки, то платеж будет находиться в статусе pending, пока не закончится период охлаждения (срок указывается в параметре suspended_until) или пока пользователь не откажется от кредита или рассрочки в приложении СберБанка.
